...
//                    Toast.makeText(getActivity(), regResult.getUserName(), Toast.LENGTH_LONG).show();
    ...
    public class signInTask extends AsyncTask<URL, Integer, Boolean> {
        ...
        protected void onPostExecute(Boolean success) {
            if(success) {
                ...
                if(RpersonId != null || RauthToken != null) {
//                    Toast.makeText(getActivity(), logResult.getUserName(), Toast.LENGTH_LONG).show();
                    syncTask sncTask = new syncTask();
                    sncTask.execute();
                }
    ...
    public class syncTask extends AsyncTask<URL, Integer, Boolean> {
        ...
        protected void onPostExecute(Boolean success) {
            if(success) {
                main.setModelData(prsnResult, prsnsResult, evntResult, logResult, regResult);
                main.mapFragSwtich();
            }
            else
                Toast.makeText(getActivity(), "Error", Toast.LENGTH_LONG).show();
